...
//
// Opt-in live Electron evidence for the Cheatly-parity gap fixes.
//
// What this exercises (via the real renderer → preload → main-process IPC):
//   1. meetingRetention round-trip (set / get / broadcast).
//   2. handleStartMeeting metadata carries doNotPersist when retention === 'never'.
//
...
// This spec proves the renderer-facing contract for the new
...
test.describe('Cheatly-parity gap E2E evidence', () => {
  test.beforeEach(() => {
    if (!E2E_ENABLED) {
  ...
  test('renderer exposes the gap-related preload APIs', async ({ page }) => {
    ...
    const apiSurface = await page.evaluate(() => {
      ...
      return {
        ...
        chatStreamStart: typeof api.chatStreamStart,
      };
    });
