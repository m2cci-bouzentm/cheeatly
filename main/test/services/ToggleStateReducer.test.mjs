...
test('off → on: changed, next=true, always broadcast', async () => {
  ...
  assert.deepEqual(decideToggle(false, true), {
    next: true,
    changed: true,
    broadcast: true,
  });
...
test('on → off: changed, next=false, always broadcast', async () => {
  const { decideToggle } = await load();
  assert.deepEqual(decideToggle(true, false), {
    next: false,
    changed: true,
    broadcast: true,
  });
...
test('REGRESSION RC-2: on → on (no-op) still broadcasts, no side-effects', async () => {
  ...
  assert.equal(
    d.changed,
    false,
    'no-op must NOT trigger expensive side-effects'
  );
  assert.equal(
    d.broadcast,
    true,
    'no-op MUST still re-broadcast to heal renderer desync'
  );
});
...
test('broadcast is invariantly true across all four transitions', async () => {
  ...
  for (const cur of [false, true]) {
    for (const req of [false, true]) {
      ...
      assert.equal(
        decideToggle(cur, req).next,
        req,
        'next always equals requested'
      );
