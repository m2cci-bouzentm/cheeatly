...
// `!app.isPackaged` AND `process.env.CHEATLY_DEV_BYPASS_SCREEN_TCC === '1'`.
...
const permissionsPath = path.join(root, 'main/utils/permissions.ts');
...
describe('B5: dev-mode TCC bypass is opt-in (CHEATLY_DEV_BYPASS_SCREEN_TCC=1) only', () => {
  it('isDevTccBypassEnabled() helper exists and checks BOTH !app.isPackaged AND the env flag', () => {
    ...
    assert.ok(
      body !== null,
      'BUG: isDevTccBypassEnabled() helper is missing from main/utils/permissions.ts. ' +
    ...
    assert.ok(
      /process\.env\.CHEATLY_DEV_BYPASS_SCREEN_TCC/.test(body),
      'BUG: isDevTccBypassEnabled() no longer checks process.env.CHEATLY_DEV_BYPASS_SCREEN_TCC. ' +
    ...
    assert.ok(
      /!\s*app\.isPackaged[\s\S]*&&[\s\S]*CHEATLY_DEV_BYPASS_SCREEN_TCC/.test(
        body
      ) ||
        /CHEATLY_DEV_BYPASS_SCREEN_TCC[\s\S]*&&[\s\S]*!\s*app\.isPackaged/.test(
          body
        ),
  ...
  it('getMacScreenCaptureStatus calls isDevTccBypassEnabled() and has no bare !app.isPackaged early-return', () => {
    ...
    assert.ok(
      body !== null,
      'could not locate getMacScreenCaptureStatus body in permissions.ts'
    );
    ...
    assert.ok(
      !/if\s*\(\s*!\s*app\.isPackaged\s*\)\s*return\s+['"]granted['"]/.test(
        body
      ),
  ...
  it('resolveMacScreenCaptureCapability calls isDevTccBypassEnabled() and has no bare !app.isPackaged early-return', () => {
    ...
    assert.ok(
      body !== null,
      'could not locate resolveMacScreenCaptureCapability body in permissions.ts'
    );
    ...
    assert.ok(
      !/if\s*\(\s*!\s*app\.isPackaged\s*\)\s*\{?\s*(?:return|clearSystemAudioPermissionWarning)/.test(
        body
      ),
  ...
  it('env var CHEATLY_DEV_BYPASS_SCREEN_TCC is documented somewhere in permissions.ts', () => {
    ...
    // escape hatch.
    const occurrences = (
      permissions.match(/CHEATLY_DEV_BYPASS_SCREEN_TCC/g) || []
    ).length;
    assert.ok(
      occurrences >= 1,
      'BUG: env var CHEATLY_DEV_BYPASS_SCREEN_TCC is no longer referenced in permissions.ts. ' +
        'This is the documented dev knob — removing it (or renaming silently) breaks the ' +
        'documented bypass workflow.'
    );
  });

  it("no function in permissions.ts returns 'granted' solely on !app.isPackaged (global audit)", () => {
    ...
    const bareBypass =
      /if\s*\(\s*!\s*app\.isPackaged\s*\)\s*\{?\s*return\s+['"]granted['"]/g;
    const matches = permissions.match(bareBypass) || [];
    assert.equal(
      matches.length,
      0,
      `BUG: permissions.ts contains ${matches.length} bare \`if (!app.isPackaged) return 'granted'\` ` +
  ...
  it("documentation block near isDevTccBypassEnabled mentions both 'granted' and the env-var name", () => {
    ...
    const idx = permissions.search(/function\s+isDevTccBypassEnabled\s*\(/);
    ...
    const docWindow = permissions.slice(Math.max(0, idx - 1500), idx);
    assert.ok(
    ...
    assert.ok(
      /CHEATLY_DEV_BYPASS_SCREEN_TCC/.test(docWindow),
      'BUG: the doc block above isDevTccBypassEnabled no longer mentions the env-var name ' +
        'CHEATLY_DEV_BYPASS_SCREEN_TCC. Devs reading the helper must be told exactly which ' +
