...
const telemetryPath = path.resolve(
  process.cwd(),
  'dist-main/main/services/telemetry/TelemetryService.js'
);
...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cheatly-telemetry-test-'));
}
...
test('API keys and tokens are redacted from properties', () => {
  const sanitized = sanitizeTelemetryProperties({
    ...
    nested: {
      token: 'sk-supersecretvalue1234567890',
  ...
  assert.doesNotMatch(serialized, /sk-supersecretvalue1234567890/);
