...
// renderer/pages/AssistantOverlay/index.tsx and
// renderer/hooks/meeting/useOverlayKeyboard.ts — the heart of PR #250 (issue #246,
...
//
//   1. blockInputFocus (renderer/hooks/meeting/useOverlayKeyboard.ts)
...
//        }, []);
//
//   2. mount-effect onMouseDown (same hook)
//        const onMouseDown = (e) => {
...
const ASSISTANT_OVERLAY = path.join(
  root,
  'renderer/pages/AssistantOverlay/index.tsx'
);
const OVERLAY_KEYBOARD = path.join(
  root,
  'renderer/hooks/meeting/useOverlayKeyboard.ts'
);
...
function shouldBlockFocus(refs) {
  // Mirrors blockInputFocus in CheatlyInterface.tsx (~line 3281).
...
function shouldFireStealthTapStart(refs) {
  // Mirrors the mount-effect onMouseDown in CheatlyInterface.tsx (~line 3223).
...
describe('overlay keyboard guard implementation must keep checking both refs', () => {
  const overlaySource = fs.readFileSync(ASSISTANT_OVERLAY, 'utf8');
  const keyboardSource = fs.readFileSync(OVERLAY_KEYBOARD, 'utf8');
  ...
  test('isCgEventTapAvailableRef defaults to false (M1 safe default)', () => {
    ...
    const refDecl = overlaySource.match(
  ...
  test('blockInputFocus checks isCgEventTapAvailableRef before preventDefault', () => {
    ...
    const body = keyboardSource.match(
      /return useCallback\([\s\S]*?shouldBlockStealthFocus[\s\S]*?\n\s*\);\n};/
    );
  ...
  test('stealthTapStart() failure is logged, not swallowed (m5)', () => {
    ...
    assert.match(
      keyboardSource,
      /stealthTapStart\(\)\s*\.catch\(\s*\(err\)\s*=>\s*console\.warn\(\s*['"]\[stealth\] tap start IPC failed['"],\s*err\s*\)\s*\)/,
  ...
  test('onStealthTapState flips isCgEventTapAvailableRef to false on permission revoke and true on active', () => {
    ...
    // both directions.
    const stateHandler = keyboardSource.match(
      /const unsubState\s*=\s*window\.electronAPI\.onStealthTapState\(\s*\(\{[\s\S]*?\}\)\s*=>\s*\{[\s\S]*?\}\s*\)\s*;/
    );
