...
const reducerPath = path.resolve(
  __dirname,
  '../../../dist-main/main/services/stealthToggleReducer.js'
);
...
test('null lastApplied → first ON transition always applies', async () => {
  ...
  assert.deepEqual(decideDockTransition(true, null), {
    shouldApply: true,
    next: true,
  });
});
...
test('null lastApplied → first OFF transition always applies', async () => {
  const { decideDockTransition } = await load();
  assert.deepEqual(decideDockTransition(false, null), {
    shouldApply: true,
    next: false,
  });
});
...
test('settled ON while dock already hidden → skip (no churn)', async () => {
  ...
  assert.equal(
    d.shouldApply,
    false,
    'must NOT re-run dock.hide() when already hidden'
  );
...
test('settled OFF while dock already shown → skip (no churn)', async () => {
  ...
  assert.equal(
    d.shouldApply,
    false,
    'must NOT re-run dock.show() when already shown'
  );
...
test('settled ON while dock currently shown → apply hide', async () => {
  const { decideDockTransition } = await load();
  assert.deepEqual(decideDockTransition(true, false), {
    shouldApply: true,
    next: true,
  });
});
...
test('settled OFF while dock currently hidden → apply show', async () => {
  const { decideDockTransition } = await load();
  assert.deepEqual(decideDockTransition(false, true), {
    shouldApply: true,
    next: false,
  });
});
...
test('rapid burst ending ON (dock was shown) → exactly one hide, matches intent', async () => {
  ...
  assert.equal(
    d.next,
    true,
    'final applied dock state equals user last intent (undetectable)'
  );
});
...
test('rapid burst returning to original state → zero dock ops', async () => {
  ...
  assert.equal(
    d.shouldApply,
    false,
    'no dock churn when net state is unchanged'
  );
});
...
test('enforce: want undetectable, OS already hidden → no re-apply (converged)', async () => {
  const { decideDockTransition } = await load();
  assert.equal(
    decideDockTransition(true, /*currentlyHidden*/ true).shouldApply,
    false
  );
});
...
test('enforce: want undetectable, OS still visible (dropped hide) → re-apply', async () => {
  ...
  assert.equal(
    decideDockTransition(true, /*currentlyHidden*/ false).shouldApply,
    true
  );
});
...
test('enforce: want detectable, OS still hidden (dropped show) → re-apply', async () => {
  const { decideDockTransition } = await load();
  assert.equal(
    decideDockTransition(false, /*currentlyHidden*/ true).shouldApply,
    true
  );
});
