...
//
// Strategy: source-level static check on main/ipc/settingsHandlers.ts.
// Importing that module boots half the app (IPC registration, BrowserWindow refs,
...
// allowlist matches the source of truth in renderer/lib/meetingInterfaceTheme.ts.
...
import { readIpcDomainSource } from './ipcTestUtils.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const themeModulePath = path.resolve(
  __dirname,
  '../../../renderer/lib/meetingInterfaceTheme.ts'
);

const ipcSource = readIpcDomainSource('settings');
...
function extractInterfaceThemeSetHandlerBody(src) {
  const sigRe =
    /safeOn\(\s*['"]interface-theme:set['"]\s*,\s*\([^)]*\)\s*=>\s*\{/;
  const m = sigRe.exec(src);
  assert.ok(
    m,
    "could not locate ipcMain.on('interface-theme:set', ...) handler"
  );
  ...
  assert.equal(
    depth,
    0,
    "unbalanced braces while extracting 'interface-theme:set' handler"
  );
...
test('settingsHandlers.ts declares VALID_INTERFACE_THEMES Set with exactly the three valid keys', () => {
  // Locate the Set declaration. Accept either `new Set([...])` or
  // `new Set<string>([...])` flavours.
  const setDeclRe =
    /VALID_INTERFACE_THEMES\s*=\s*new\s+Set\s*(?:<[^>]+>)?\s*\(\s*\[([^\]]+)\]\s*\)/;
  const m = setDeclRe.exec(ipcSource);
  assert.ok(
    m,
    'BUG: settingsHandlers.ts must declare `VALID_INTERFACE_THEMES = new Set([...])` ' +
...
test('guard returns BEFORE BrowserWindow.getAllWindows().forEach broadcast', () => {
  const guardIdx = handlerBody.search(
    /!\s*VALID_INTERFACE_THEMES\.has\s*\(\s*theme\s*\)/
  );
  const broadcastIdx = handlerBody.search(
    /BrowserWindow\.getAllWindows\s*\(\s*\)\s*\.forEach/
  );

  assert.ok(guardIdx >= 0, 'allowlist guard missing from handler body');
  assert.ok(
    broadcastIdx >= 0,
    'BrowserWindow.getAllWindows().forEach broadcast missing from handler body'
  );
  assert.ok(
...
test('settingsHandlers.ts allowlist matches renderer/lib/meetingInterfaceTheme.ts source of truth', () => {
  ...
  assert.ok(
    aliasMatch,
    'could not locate MeetingInterfaceTheme type alias in meetingInterfaceTheme.ts'
  );
  const aliasLiterals = [...aliasMatch[1].matchAll(/['"]([^'"]+)['"]/g)].map(
    (mm) => mm[1]
  );
  assert.deepEqual(
    ...
    `MeetingInterfaceTheme type alias drifted from expected ${JSON.stringify(EXPECTED_THEMES)}: ` +
      ...
      'and the settingsHandlers.ts allowlist together.'
  );

  const validThemesRe =
    /VALID_THEMES\s*:\s*[^=]+=\s*new\s+Set\s*(?:<[^>]+>)?\s*\(\s*\[([^\]]+)\]\s*\)/;
  const validThemesMatch = validThemesRe.exec(themeSource);
  assert.ok(
    validThemesMatch,
    'could not locate VALID_THEMES Set in meetingInterfaceTheme.ts'
  );
  const validThemesLiterals = [
    ...validThemesMatch[1].matchAll(/['"]([^'"]+)['"]/g),
  ].map((mm) => mm[1]);
  assert.deepEqual(
  ...
  const ipcSetMatch =
    /VALID_INTERFACE_THEMES\s*=\s*new\s+Set\s*(?:<[^>]+>)?\s*\(\s*\[([^\]]+)\]\s*\)/.exec(
      ipcSource
    );
  assert.ok(
    ipcSetMatch,
    'settingsHandlers.ts VALID_INTERFACE_THEMES declaration not found'
  );
  const ipcLiterals = [...ipcSetMatch[1].matchAll(/['"]([^'"]+)['"]/g)].map(
    (mm) => mm[1]
  );
  ...
  assert.deepEqual(
    ...
    validThemesLiterals.slice().sort(),
    'BUG: settingsHandlers.ts VALID_INTERFACE_THEMES is out of sync with ' +
      'renderer/lib/meetingInterfaceTheme.ts VALID_THEMES. The two allowlists must ' +
