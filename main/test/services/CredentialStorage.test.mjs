...
test('CredentialsManager does not persist plaintext fallback credentials when encryption is unavailable', () => {
  const source = read('main/services/CredentialsManager.ts');
  ...
  assert.match(
    saveSource,
    /Encryption not available; credentials kept in memory only/
  );
...
test('CredentialsManager removes plaintext fallback files instead of loading them', () => {
  ...
  const plaintextSectionStart = loadSource.indexOf(
    "const plaintextPath = CREDENTIALS_PATH + '.json';",
    loadSource.indexOf('// Try encrypted file first') + 1
  );
...
test('SettingsManager does not log full settings JSON', () => {
  const source = read('main/services/SettingsManager.ts');

  assert.match(
    source,
    /Settings loaded successfully['\"],\s*\{\s*keys:\s*Object\.keys\(this\.settings\)\.length/
  );
