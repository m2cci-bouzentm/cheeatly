...
//
// Root cause: src/components/CheatlyInterface.tsx generated React list-item
// keys via `Date.now().toString()` at ~30 different call sites. Several
// handlers (handleManualSubmit, handleWhatToSay, error fallbacks, etc.) append
// two messages back-to-back in a single synchronous tick — both
...
test('Date.now().toString() collides when called twice in one tick (documents the bug)', () => {
  ...
  assert.equal(
    a,
    b,
    'Date.now() returned in the same tick produces identical IDs'
  );
});
