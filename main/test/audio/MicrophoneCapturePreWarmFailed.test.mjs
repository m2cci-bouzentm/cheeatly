...
test('MicrophoneCapture emits pre_warm_failed when deferred pre-warm constructor throws', async () => {
  ...
  assert.equal(
    micInstanceCount,
    1,
    'eager init should have constructed the first native instance'
  );
  ...
  assert.equal(
    micInstanceCount,
    1,
    'start() must not construct a new native instance'
  );
  ...
  assert.ok(
    ...
    `'pre_warm_failed' payload must be an Error, got ${typeof err}`
  );
  assert.match(
    err.message,
    /simulated/,
