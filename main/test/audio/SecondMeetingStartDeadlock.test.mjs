...
const meetingImplPath = path.resolve(
  __dirname,
  '../../../main/services/meeting/MeetingImpl.ts'
);
const meetingImplSource = readFileSync(meetingImplPath, 'utf8');
...
function deviceFor(id) {
  ...
  if (!deviceState.has(key))
    deviceState.set(key, { liveStops: 0, constructedWhileStopping: false });
...
const fakeNativeModule = {
  ...
  getInputDevices: () => [],
  getOutputDevices: () => [],
  SystemAudioCapture: function (d) {
    return makeFakeMic('sys:' + (d || 'default'));
  },
  MicrophoneCapture: function (d) {
    return makeFakeMic('mic:' + (d || 'default'));
  },
};
...
Module._load = function patched(request, _parent, _isMain) {
  ...
  if (request.endsWith('.node') || request.includes('platform-bridge')) {
...
const { MicrophoneCapture } = await import(
  path.join(distRoot, 'MicrophoneCapture.js')
);
...
test('meeting-2 mic must not be constructed until meeting-1 destroy() resolves (no HAL overlap)', async () => {
  ...
  assert.ok(
    dev.liveStops > 0,
    'native monitor.stop() must be in flight after the setImmediate fires'
  );
  ...
  assert.equal(
    teardownResolved,
    true,
    'destroy() promise must resolve after the native stop is released'
  );
  assert.equal(
    dev.liveStops,
    0,
    'native teardown must be fully drained before we construct meeting-2'
  );
...
function extractFunctionBody(functionName) {
  ...
  const m = re.exec(meetingImplSource);
  assert.ok(m, `could not locate ${functionName} in MeetingImpl.ts`);
  let i = m.index + m[0].length;
  let depth = 1;
  const start = i;
  while (i < meetingImplSource.length && depth > 0) {
    const ch = meetingImplSource[i];
    if (ch === '{') depth++;
    if (ch === '}') depth--;
    i++;
  }
  assert.equal(depth, 0, `unbalanced braces in ${functionName}`);
  return meetingImplSource.slice(start, i - 1);
}

const endMeetingBody = extractFunctionBody('endMeetingImpl');
const startMeetingBody = extractFunctionBody('startMeetingImpl');

test('endMeeting NULLS both capture fields synchronously (forces serialized recreate path)', () => {
  assert.ok(
    /this\.audioService\.systemAudioCapture\s*=\s*null/.test(endMeetingBody),
    'BUG: endMeeting must null this.audioService.systemAudioCapture so the next meeting reconstructs it via the serialized path instead of reusing a half-torn-down wrapper.'
  );
  assert.ok(
    /this\.audioService\.microphoneCapture\s*=\s*null/.test(endMeetingBody),
    'BUG: endMeeting must null this.audioService.microphoneCapture — otherwise reconfigureAudio early-returns "device IDs unchanged" and MicrophoneCapture.start() synchronously constructs a fresh native mic on the main thread, racing the dying monitor.stop() (the HAL deadlock).'
  );
});
...
test('endMeeting threads capture teardown into _pendingTeardown, awaited UP FRONT', () => {
  ...
  // settles before any later work, and before the next meeting which awaits
  // this whole promise constructs a capture).
  const pendingIdx = endMeetingBody.search(
    /this\._pendingTeardown\s*=\s*\(\s*async/
  );
  assert.ok(
    pendingIdx >= 0,
    'sanity: endMeeting assigns this._pendingTeardown to an async IIFE'
  );
  const pendingBody = endMeetingBody.slice(pendingIdx);

  const awaitCapIdx = pendingBody.search(/await\s+captureTeardownPromise/);
  // finalize() is the drain-ordered stop (per-channel stop + awaitDrained).
  const sttStopIdx = pendingBody.search(
    /this\.audioService\.interviewerSTT\?\.\s*(?:finalize|stop)/
  );
  assert.ok(
    awaitCapIdx >= 0,
  ...
  assert.ok(
    sttStopIdx >= 0,
    'sanity: the _pendingTeardown IIFE stops STT later'
  );
  assert.ok(
...
test('startMeeting awaits _pendingTeardown BEFORE the async audio init', () => {
  ...
  const audioInitIdx = startMeetingBody.search(
    /this\._audioInitPromise\s*=\s*\(\s*async/
  );
  assert.ok(
    awaitIdx >= 0,
    "BUG: startMeeting must await this._pendingTeardown so the prior meeting's capture teardown completes before a new capture is constructed."
  );
  assert.ok(
    audioInitIdx >= 0,
    'sanity: startMeeting assigns this._audioInitPromise'
  );
  assert.ok(
