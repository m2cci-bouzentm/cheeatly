...
//
// Root cause:
//   - `main/main.ts` `reconfigureAudio()` short-circuits when device IDs
...
Module._load = function patchedLoad(request, parent, isMain) {
  if (request === 'electron') {
    return {
      app: {
        getAppPath: () => '/tmp/fake-cheatly-app',
        isPackaged: false,
        isReady: () => false,
      },
    };
  ...
  //   <appPath>/platform-bridge/index.<platform>-<arch>.node
  // We intercept any require for a path ending in `.node` AND containing
  // `platform-bridge` so the loader's first attempt succeeds.
  if (
    typeof request === 'string' &&
    request.endsWith('.node') &&
    request.includes('platform-bridge')
  ) {
...
const { SystemAudioCapture } = await import(
  path.join(distRoot, 'SystemAudioCapture.js')
);
...
test('SystemAudioCapture restart after stop must not reuse a torn-down native monitor', async () => {
  ...
  assert.equal(
    ...
    1,
    'first start should construct a native instance'
  );
  const first = created.system[0];
  assert.equal(
    first.startCalls,
    1,
    'first native instance should have been started exactly once'
  );
  ...
  assert.equal(
    first.stopCalls,
    1,
    'deferred native stop() should have run after setImmediate flush'
  );
  assert.equal(
    first.torndown,
    true,
    'first native instance must be marked torn down'
  );
  ...
  assert.notStrictEqual(
    second,
    first,
    'second native instance must be a distinct object'
  );
  assert.equal(
    second.torndown,
    false,
    'fresh native instance must not be torn down'
  );
  assert.equal(
    second.startCalls,
    1,
    'fresh native instance should be started exactly once'
  );
...
test('MicrophoneCapture restart after stop must not reuse a torn-down native monitor', async () => {
  ...
  assert.equal(
    created.microphone.length,
    1,
    'constructor should eagerly create a native instance'
  );
  ...
  assert.equal(
    first.torndown,
    true,
    'first native mic instance must be marked torn down'
  );
  ...
  assert.notStrictEqual(
    second,
    first,
    'second native mic instance must be a distinct object'
  );
  assert.equal(
    second.torndown,
    false,
    'fresh native mic instance must not be torn down'
  );
  assert.equal(
    second.startCalls,
    1,
    'fresh native mic instance should be started exactly once'
  );
