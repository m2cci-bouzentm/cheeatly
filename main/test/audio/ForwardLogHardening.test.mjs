...
import { readIpcDomainSource } from '../services/ipcTestUtils.mjs';
...
function extractForwardLogHandler() {
  ...
  assert.ok(
    start >= 0,
    'could not locate forward-log-to-file safeOn registration'
  );
  let i = ipcSource.indexOf('{', start);
  assert.ok(
    i >= 0,
    'could not find opening brace for forward-log-to-file handler'
  );
  let depth = 1;
  const bodyStart = i + 1;
  i++;
  ...
  assert.equal(
    depth,
    0,
    'unbalanced braces while extracting forward-log-to-file handler'
  );
...
test('forward-log-to-file validates types, scrubs control characters, caps length, rate-limits, and tags sender', () => {
  ...
  assert.ok(
    /typeof\s+level\s*!==\s*['"]string['"][\s\S]*typeof\s+msg\s*!==\s*['"]string['"]/.test(
      body
    ),
    'BUG: forward-log-to-file must reject non-string level/msg payloads from renderers.'
  );
  assert.ok(
    /msg\s*\.replace\s*\(\s*\/\[\\r\\n\\x00[\s\S]+?,\s*['"] ['"]\s*\)\s*\.slice\s*\(\s*0\s*,\s*FORWARD_LOG_MAX_LEN\s*\)/.test(
      body
    ),
  ...
  assert.ok(
    /Math\.floor\(\s*\(elapsed\s*\*\s*FORWARD_LOG_RATE_BUCKET\)\s*\/\s*FORWARD_LOG_RATE_REFILL_MS\s*\)/.test(
      body
    ),
  ...
  assert.ok(
    /event\.sender\?\.\s*id\s*\?\?\s*-1/.test(body) &&
      /\[\$\{senderId\}\]/.test(body),
