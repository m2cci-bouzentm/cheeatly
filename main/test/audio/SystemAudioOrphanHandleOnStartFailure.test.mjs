...
const fakeNativeModule = {
  ...
  MicrophoneCapture: function () {
    return { start() {}, stop() {}, getSampleRate: () => 48000 };
  },
};
...
test('SystemAudioCapture.start() failure must stop the orphaned native monitor', async () => {
  ...
  assert.equal(
    ...
    1,
    'lazy init should construct exactly one native instance'
  );
...
test('After failed start + orphan cleanup, a retry start() constructs a fresh native instance', async () => {
  ...
  assert.equal(
    first.torndown,
    true,
    'first instance was torn down after orphan cleanup'
  );
  ...
  assert.equal(
    second.startCalls,
    1,
    'fresh instance has been started exactly once'
  );
