...
//
// Strategy: reuse the fake-platform-bridge harness from
...
test('MicrophoneCapture.stop() returns a Promise that resolves after native teardown', async () => {
  ...
  assert.equal(
    cap.isRecording,
    false,
    'isRecording must flip synchronously inside stop()'
  );
  assert.equal(
    first.stopCalls,
    0,
...
test('Promise<void> shape: stop() can be safely fire-and-forget (no unhandled-rejection regression)', async () => {
  ...
  assert.equal(
    first.stopCalls,
    1,
    'fire-and-forget stop() must still tear down cheatly after the event loop drains'
  );
