...
const mix = (min: number, max: number, value: number) =>
  min + (max - min) * value;
...
export const clampOverlayOpacity = (opacity: number) =>
  clamp(opacity, OVERLAY_OPACITY_MIN, OVERLAY_OPACITY_MAX);

const normalizeOpacity = (opacity: number) =>
  (clampOverlayOpacity(opacity) - OVERLAY_OPACITY_MIN) /
...
export const getOverlayAppearance = (opacity: number): OverlayAppearance => {
