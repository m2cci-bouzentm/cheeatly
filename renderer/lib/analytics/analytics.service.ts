// Renderer-only GA4 injection uses a public measurement ID; no API secrets.

export type ModelProviderType = 'cloud' | 'local';

type AssistantMode = 'launcher' | 'overlay' | 'undetectable' | string;
...
class AnalyticsService {
  ...
  public trackAppOpen(): void {
    ...
    const hasLaunched = localStorage.getItem('cheatly_has_launched');
    if (!hasLaunched) {
      this.trackEvent('first_launch');
      localStorage.setItem('cheatly_has_launched', 'true');
    }
  }
