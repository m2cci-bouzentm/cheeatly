...
function detectHardwareArch() {
  ...
  try {
    const isArm = execFileSync('sysctl', ['-in', 'hw.optional.arm64'], {
      encoding: 'utf8',
    }).trim();
...
function canRunNodeGypPython(python) {
  ...
  try {
    execFileSync(python, ['-c', 'import plistlib'], { stdio: 'ignore' });
    return true;
  } catch {
    return false;
  }
...
function resolveNodeGypPython() {
  ...
  for (const python of [...new Set(candidates.filter(Boolean))]) {
...
function main() {
  ...
  if (electronVersion === null) {
    console.warn(
      '[rebuild-native] electron not installed yet — skipping native rebuild.'
    );
    return;
  }
  ...
  if (!fs.existsSync(cli)) {
    console.warn(
      `[rebuild-native] @electron/rebuild v4 CLI not found at ${cli} — skipping.`
    );
    return;
  }
  ...
  const cmdArgs = useArchWrapper
    ? [`-${arch === 'x64' ? 'x86_64' : arch}`, process.execPath, ...args]
    : args;
  const env = { ...process.env };
  const python = resolveNodeGypPython();

  if (python) {
    env.PYTHON = python;
    console.log(`[rebuild-native] Using node-gyp Python: ${python}`);
  }

  execFileSync(cmd, cmdArgs, { stdio: 'inherit', cwd: root, env });
