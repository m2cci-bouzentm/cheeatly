...
const platformBridgePath = path.join(__dirname, '..', 'platform-bridge');
const buildAllMacTargets = process.env.CHEATLY_BUILD_ALL_MAC_ARCHES === '1';

function verifyArtifacts(expectedArtifacts) {
  const missing = expectedArtifacts.filter(
    (file) => !fs.existsSync(path.join(platformBridgePath, file))
  );
...
const isMacBuild = os.platform() === 'darwin';
if (isMacBuild) {
  ...
  clangLibPath && console.log(`Using clang runtime path: ${clangLibPath}`);
  ...
  for (const target of macTargets) {
    ...
    const artifactPath = path.join(platformBridgePath, artifact);
...
const isCurrentPlatformBuild = !isMacBuild;
if (isCurrentPlatformBuild) {
  ...
  expectedArtifacts && verifyArtifacts(expectedArtifacts);
}
